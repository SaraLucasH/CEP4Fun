...
Blockly.Blocks.lists_split = {
    ...
    updateType_: function (a) {
        ...
        "SPLIT" == a ? (this.outputConnection.setCheck("Array"), this.getInput("INPUT").setCheck("String")) :
            (this.outputConnection.setCheck("String"), this.getInput("INPUT").setCheck("Array"))
    },
    mutationToDom: function () {
        var a = Blockly.utils.xml.createElement("mutation");
...
Blockly.Constants.Logic.CONTROLS_IF_MUTATOR_MIXIN = {
    ...
    compose: function (a) {
        ...
        for (var b = [null], c = [null], e = null; a;) {
            switch (a.type) {
        ...
        this.updateShape_();
        this.reconnectChildBlocks_(b, c, e)
    },
    saveConnections: function (a) {
        a = a.nextConnection.targetBlock();
        for (var b = 1; a;) {
    ...
    rebuildShape_: function () {
        ...
        for (var e = 1; this.getInput("IF" + e);) {
            var d = this.getInput("IF" + e),
                f = this.getInput("DO" +
                    e);
            a.push(d.connection.targetConnection);
    ...
    reconnectChildBlocks_: function (a, b, c) {
        for (var e = 1; e <= this.elseifCount_; e++) Blockly.Mutator.reconnect(a[e], this, "IF" + e), Blockly.Mutator.reconnect(b[e], this, "DO" + e);
...
Blockly.Constants.Logic.LOGIC_COMPARE_ONCHANGE_MIXIN = {
    onchange: function (a) {
        ...
        b && c && !b.outputConnection.checkType(c.outputConnection) && (Blockly.Events.setGroup(a.group), a = this.prevBlocks_[0], a !== b && (b.unplug(), !a || a.isDisposed() || a.isShadow() || this.getInput("A").connection.connect(a.outputConnection)), b = this.prevBlocks_[1], b !== c && (c.unplug(), !b || b.isDisposed() || b.isShadow() || this.getInput("B").connection.connect(b.outputConnection)),
...
Blockly.Constants.Logic.LOGIC_TERNARY_ONCHANGE_MIXIN = {
    ...
    onchange: function (a) {
        ...
        if ((b || c) && e)
            for (var d = 0; 2 > d; d++) {
                var f = 1 == d ? b : c;
                f && !f.outputConnection.checkType(e) && (Blockly.Events.setGroup(a.group), e === this.prevParentConnection_ ? (this.unplug(), e.getSourceBlock().bumpNeighbours()) : (f.unplug(), f.bumpNeighbours()), Blockly.Events.setGroup(!1))
            }
    ...
    customContextMenu: function (a) {
        if (!this.isInFlyout) {
            ...
            if (!this.isCollapsed() && null != c) {
                ...
                e.text = Blockly.Msg.VARIABLES_SET_CREATE_GET.replace("%1", c);
...
Blockly.Constants.Loops.CONTROL_FLOW_IN_LOOP_CHECK_MIXIN = {
    ...
    onchange: function (a) {
        this.workspace.isDragging && !this.workspace.isDragging() && (Blockly.Constants.Loops.CONTROL_FLOW_IN_LOOP_CHECK_MIXIN.getSurroundLoop(this) ?
            (this.setWarningText(null), this.isInFlyout || this.setEnabled(!0)) : (this.setWarningText(Blockly.Msg.CONTROLS_FLOW_STATEMENTS_WARNING), this.isInFlyout || this.getInheritedDisabled() || this.setEnabled(!1)))
    }
};
...
Blockly.Blocks.procedures_defnoreturn = {
    ...
    domToMutation: function (a) {
        ...
        for (var b = 0, c; c = a.childNodes[b]; b++)
            if ("arg" == c.nodeName.toLowerCase()) {
                ...
                c = Blockly.Variables.getOrCreateVariablePackage(this.workspace, c,
                    e, "");
                null != c ? this.argumentVarModels_.push(c) : console.log("Failed to create a variable with name " + e + ", ignoring.")
            } this.updateParams_();
    ...
    decompose: function (a) {
        ...
        for (var e = 0; e < this.arguments_.length; e++) {
            ...
            var g = Blockly.utils.xml.createTextNode(this.arguments_[e]);
            f.appendChild(g);
            d.appendChild(f);
    ...
    renameVarById: function (a, b) {
        ...
        if ("" == c.type) {
            ...
            for (var e = !1, d = 0; d < this.argumentVarModels_.length; d++) this.argumentVarModels_[d].getId() == a && (this.arguments_[d] = b.name, this.argumentVarModels_[d] = b, e = !0);
            e && (this.displayRenamedVar_(c, b.name), Blockly.Procedures.mutateCallers(this))
        }
    },
    updateVarName: function (a) {
        for (var b = a.name, c = !1, e = 0; e < this.argumentVarModels_.length; e++)
            if (this.argumentVarModels_[e].getId() == a.getId()) {
                var d = this.arguments_[e];
                this.arguments_[e] = b;
                c = !0
            } c && (this.displayRenamedVar_(d,
            b), Blockly.Procedures.mutateCallers(this))
    },
    displayRenamedVar_: function (a, b) {
        this.updateParams_();
        if (this.mutator && this.mutator.isVisible())
            for (var c = this.mutator.workspace_.getAllBlocks(!1), e = 0, d; d = c[e]; e++) "procedures_mutatorarg" == d.type && Blockly.Names.equals(a, d.getFieldValue("NAME")) && d.setFieldValue(b, "NAME")
    },
    customContextMenu: function (a) {
        if (!this.isInFlyout) {
            var b = {
            ...
            if (!this.isCollapsed())
                for (c = 0; c < this.argumentVarModels_.length; c++) b = {
                        enabled: !0
                    }, e = this.argumentVarModels_[c], b.text = Blockly.Msg.VARIABLES_SET_CREATE_GET.replace("%1", e.name),
                    e = Blockly.Variables.generateVariableFieldDom(e), d = Blockly.utils.xml.createElement("block"), d.setAttribute("type", "variables_get"), d.appendChild(e), b.callback = Blockly.ContextMenu.callbackFactory(this, d), a.push(b)
        }
    },
...
Blockly.Blocks.procedures_mutatorarg = {
    ...
    validator_: function (a) {
        ...
        if (!a) return null;
        for (var e = (b.workspace.targetWorkspace || b.workspace).getAllBlocks(!1), d = 0; d < e.length; d++)
            if (e[d].id != this.getSourceBlock().id && e[d].getFieldValue("NAME") == a) return null;
        if (b.isInFlyout) return a;
        (b = c.getVariable(a, "")) && b.name != a && c.renameVariableById(b.getId(),
            a);
    ...
    deleteIntermediateVars_: function (a) {
        ...
        if (b)
            for (var c = 0; c < this.createdVariables_.length; c++) {
                var e = this.createdVariables_[c];
                e.name != a && b.deleteVariableById(e.getId())
            }
    }
};
Blockly.Blocks.procedures_callnoreturn = {
        ...
        if (b)
            if (a.join("\n") == this.arguments_.join("\n")) this.quarkIds_ = b;
                ...
                for (var d = 0; d < this.arguments_.length; d++) {
                    ...
                    f && (f = f.connection.targetConnection, this.quarkConnections_[this.quarkIds_[d]] = f, e && f && -1 == b.indexOf(this.quarkIds_[d]) && (f.disconnect(), f.getSourceBlock().bumpNeighbours()))
                }
                this.arguments_ = [].concat(a);
                this.argumentVarModels_ = [];
                for (d = 0; d < this.arguments_.length; d++) a = Blockly.Variables.getOrCreateVariablePackage(this.workspace,
                    null, this.arguments_[d], ""), this.argumentVarModels_.push(a);
                this.updateShape_();
                if (this.quarkIds_ = b)
                    for (d = 0; d < this.arguments_.length; d++) b = this.quarkIds_[d], b in this.quarkConnections_ && (f = this.quarkConnections_[b], Blockly.Mutator.reconnect(f, this, "ARG" + d) || delete this.quarkConnections_[b]);
    ...
    domToMutation: function (a) {
        ...
        b = [];
        for (var c = [], e = 0, d; d = a.childNodes[e]; e++) "arg" == d.nodeName.toLowerCase() && (b.push(d.getAttribute("name")), c.push(d.getAttribute("paramId")));
    ...
    customContextMenu: function (a) {
        if (this.workspace.isMovable()) {
            ...
            b.callback = function () {
                ...
                a && (e.centerOnBlock(a.id), a.select())
            };
            a.push(b)
        }
    },
...
Blockly.Blocks.text_getSubstring = {
    ...
    updateAt_: function (a, b) {
        ...
        this.getInput("AT" + a).appendField(c, "WHERE" + a);
        1 == a && (this.moveInputBefore("AT1", "AT2"), this.getInput("ORDINAL1") &&
...
Blockly.Constants.Text.QUOTE_IMAGE_MIXIN = {
    ...
    quoteField_: function (a) {
        for (var b = 0, c; c = this.inputList[b]; b++)
            for (var e = 0, d; d = c.fieldRow[e]; e++)
                if (a == d.name) {
                    c.insertFieldAt(e, this.newQuote_(!0));
                    c.insertFieldAt(e + 2, this.newQuote_(!1));
                    return
...
Blockly.Constants.Text.TEXT_CHARAT_EXTENSION = function () {
    ...
    this.setTooltip(function () {
        ...
        ("FROM_START" == b || "FROM_END" == b) && (b = "FROM_START" == b ? Blockly.Msg.LISTS_INDEX_FROM_START_TOOLTIP : Blockly.Msg.LISTS_INDEX_FROM_END_TOOLTIP) && (c += "  " + b.replace("%1", a.workspace.options.oneBasedIndex ?
            "#1" : "#0"));
        return c
    })
};
...
Blockly.Constants.Variables.CUSTOM_CONTEXT_MENU_VARIABLE_GETTER_SETTER_MIXIN = {
    customContextMenu: function (a) {
        if (!this.isInFlyout) {
            ...
            d.appendChild(c);
            e.callback = Blockly.ContextMenu.callbackFactory(this, d);
            a.push(e)
        } else if ("variables_get" == this.type || "variables_get_reporter" == this.type) b = {
                text: Blockly.Msg.RENAME_VARIABLE,
                enabled: !0,
                callback: Blockly.Constants.Variables.RENAME_OPTION_CALLBACK_FACTORY(this)
            }, d = this.getField("VAR").getText(), e = {
                text: Blockly.Msg.DELETE_VARIABLE.replace("%1", d),
                enabled: !0,
    ...
    customContextMenu: function (a) {
        if (!this.isInFlyout) {
            ...
            if ("variables_get_dynamic" == this.type) {
                ...
                var e = Blockly.Msg.VARIABLES_GET_CREATE_SET
            } else b = "variables_get_dynamic", e = Blockly.Msg.VARIABLES_SET_CREATE_GET;
            var d = {
                    enabled: 0 < this.workspace.remainingCapacity()
                },
                f = this.getField("VAR").getText();
            d.text = e.replace("%1", f);
            e = Blockly.utils.xml.createElement("field");
            e.setAttribute("name", "VAR");
            e.setAttribute("variabletype", c);
            e.appendChild(Blockly.utils.xml.createTextNode(f));
            f = Blockly.utils.xml.createElement("block");
            f.setAttribute("type", b);
            f.appendChild(e);
            d.callback = Blockly.ContextMenu.callbackFactory(this, f);
            a.push(d)
