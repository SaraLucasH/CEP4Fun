...
function toXml() {
  ...
  var logEvento = new LogEventButtonBlockly("seeXML",workspace);
...
/*
  ...
  */
function saveXml(){
    var logEvento = new LogEventButtonBlockly("downloadXML",workspace);
    logsEventos.push(logEvento); 
    var xml = Blockly.Xml.workspaceToDom(workspace);
...
{
  var logEvento = new LogEventButtonBlockly("importXML",workspace);
  logsEventos.push(logEvento); 
...
function registerLogs(state){
  ...
  if (state) {
    buttonSaveLog.disabled = false;
    workspace.addChangeListener(logger);
    logEvento1 = new LogEventButtonBlockly("activateLogEvent",workspace);
    logsEventos.push(logEvento1);
  } else {
    workspace.removeChangeListener(logger);
    flyoutWorkspace.removeChangeListener(logger);
    logEvento1 = new LogEventButtonBlockly("disableLogEvent",workspace);
    logsEventos.push(logEvento1);
